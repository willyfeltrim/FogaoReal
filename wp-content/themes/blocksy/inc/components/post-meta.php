	...
	function blocksy_post_meta($post_meta_descriptor = null, $args = []) {
		...
		foreach ($post_meta_descriptor as $single_meta) {
			...
			if (
				...
				$maybe_taxonomy
				&&
				blocksy_get_categories_list('', $maybe_taxonomy)
			) {
				...
				echo blocksy_get_categories_list($divider, $maybe_taxonomy);
...
if (! function_exists('blocksy_get_the_term_list')) {
	function blocksy_get_the_term_list(
		$post_id,
		$taxonomy,
		$before = '',
		$sep = '',
		...
		foreach ($terms as $term) {
			...
			$links[] = blocksy_html_tag(
				...
				[
					'href' => $link,
					'rel' => 'tag',
					'class' => 'ct-term-' . $term->term_id
				],
				$before_each . $term->name
			);
		}
...
if (! function_exists('blocksy_get_categories_list')) {
	function blocksy_get_categories_list($between = '', $taxonomy = null, $before_each = '') {
		global $post;
		...
		if (! $taxonomy) {
			$taxonomy = blocksy_maybe_get_matching_taxonomy($post_type);
		}
